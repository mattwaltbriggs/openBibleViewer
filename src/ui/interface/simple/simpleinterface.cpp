...
{
    DEBUG_FUNC_NAME
    QString cssPath = m_settings->getModuleSettings(m_module->moduleID())->styleSheet;
    if(cssPath.isEmpty())
        cssPath = ":/data/css/default.css";
    m_view->setHtml(text);
    {
        QFile cssFile(cssPath.startsWith("qrc:") ? ":" + cssPath.mid(4) : cssPath);
