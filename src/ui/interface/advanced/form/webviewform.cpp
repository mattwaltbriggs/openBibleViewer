...
void WebViewForm::saveFile()
{
    ...
    if(fi.suffix().compare("html", Qt::CaseInsensitive) == 0 ||
        ...
        m_view->page()->toHtml([&out](const QString &html){ out << html; });
        file.close();
        ...
        QTextDocument doc;
        m_view->page()->toHtml([&doc](const QString &html){ doc.setHtml(html); });
        writer.write(&doc);
    } else {
        QFile file(fileName);
        if(!file.open(QIODevice::WriteOnly | QIODevice::Text))
...
void WebViewForm::loadStyleSheet(QString url)
{
    ...
    */
    ...
    {
        QString cssUrl = url;
        QFile cssFile(cssUrl.startsWith("qrc:") ? ":" + cssUrl.mid(4) : cssUrl);
        if(cssFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
            QString css = QString::fromUtf8(cssFile.readAll());
            QString js = QString::fromLatin1(
                "(function() {"
