...
void BibleForm::restore(const QString &key)
{
    ...
    m_view->page()->runJavaScript(QString("window.scrollTo(%1, %2);").arg(scroll.x()).arg(scroll.y()));
...
void BibleForm::save()
{
    ...
    m_settings->session.file()->setValue(a + "scrollPosition", QPoint());
...
void BibleForm::setBooks(QSharedPointer<Versification> v11n)
{
    ...
    // Sort by key to ensure consistent ordering (QHash::keys/values don't guarantee matching order in Qt6)
    QList<int> sortedKeys = books.keys();
    std::sort(sortedKeys.begin(), sortedKeys.end());
    QStringList sortedNames;
    m_bookIDs.clear();
    for(int key : sortedKeys) {
        sortedNames << books.value(key);
        m_bookIDs << key;
    }

    bool same = true;
    if(m_ui->comboBox_books->count() == sortedNames.count()) {
        for(int j = 0; j < sortedNames.count(); ++j) {
            if(m_ui->comboBox_books->itemText(j) != sortedNames.at(j)) {
                same = false;
                break;
            }
        }
    ...
    if(!same) {
        ...
        m_ui->comboBox_books->insertItems(0, sortedNames);
    }
...
void BibleForm::activated()
{
    ...
    m_api->moduleApi()->setWebView(m_view);
    if(m_verseTable == nullptr) {
        clearChapters();
        clearBooks();
        m_actions->setTitle("");
...
void BibleForm::showText(const QString &text)
{
    ...
    if(verseModule()->moduleType() == ModuleTools::BibleQuoteModule) {
        ...
        m_view->page()->runJavaScript("document.querySelectorAll('img')", [searchPaths](const QVariant &result) {
            Q_UNUSED(result);
        });
    }
...
void BibleForm::showContextMenu(QContextMenuEvent* ev)
{
    ...
    std::sort(list.begin(), list.end(), ModuleManager::sortModuleByPop);
    ...
    m_view->page()->runJavaScript(
        ...
        [this, text](const QVariant &v){
            ...
            if(doc.isObject()) {
                ...
                selection.moduleID = obj.value("moduleID").toInt(-1);
                selection.bookID = obj.value("bookID").toInt(-1);
                selection.startChapterID = obj.value("startChapterID").toInt(-1);
                selection.endChapterID = obj.value("endChapterID").toInt(-1);
...
void BibleForm::removeMark()
{
    ...
    if(s.canBeUsedForMarks() == false && verseModule()->moduleType() != ModuleTools::BibleQuoteModule) {
        ...
        m_view->page()->runJavaScript("var adVerseSelection = new AdVerseSelection(); adVerseSelection.getSelect();");
        QString startVerseText2;
        m_view->page()->runJavaScript("adVerseSelection.startVerseText;", [&startVerseText2](const QVariant &v){ startVerseText2 = v.toString(); });
       /* myDebug() << "adVerseSelection.startVerse" << f->evaluateJavaScript("adVerseSelection.startVerse;").toString();
        ...
        m_view->page()->runJavaScript("removeSelectionStuff()");
