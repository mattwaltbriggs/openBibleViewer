...
bool BibleLink::fastIsBibleLink(const QString &s)
{
    ...
    int found = -1;
    QRegularExpression foundRegExp;
    for(int i = reg.size() - 1; i >= 0; --i) {
        QRegularExpression r(reg.at(i));
...
bool BibleLink::isBibleLink(const QString &s)
{
    ...
    if(found == 0) {
        int lev = 0;
        QRegularExpressionMatch match0 = foundRegExp.match(s);
        bookNameToBookID(match0.captured(1), &lev);
        myDebug() << "lev = " << lev;
        if(lev < 2)
            return true;
        else
...
VerseUrl BibleLink::getUrl(const QString& s)
{
    ...
    int lev = 0;
    QRegularExpressionMatch foundMatch = foundRegExp.match(s);
    ...
    if(found == 0) {  //example: Hiob
        const int bookID = bookNameToBookID(foundMatch.captured(1), &lev);
        ...
        const int bookID = bookNameToBookID(foundMatch.captured(1), &lev);
        const int chapterID = foundMatch.captured(3).toInt() - 1;
        range.setBook(bookID);
        ...
        const int bookID = bookNameToBookID(foundMatch.captured(1), &lev);
        const int chapterID = foundMatch.captured(3).toInt() - 1;
        const int verseID = foundMatch.captured(4).toInt() - 1;
        range.setBook(bookID);
        range.setChapter(chapterID);
        ...
        const int bookID = bookNameToBookID(foundMatch.captured(1), &lev);
        const int chapterID = foundMatch.captured(3).toInt() - 1;
        const int startVerseID = foundMatch.captured(4).toInt() - 1;
        const int endVerseID = foundMatch.captured(5).toInt() - 1;
        range.setBook(bookID);
        range.setChapter(chapterID);
        ...
        const int bookID = bookNameToBookID(foundMatch.captured(1), &lev);
        const int startChapterID = foundMatch.captured(3).toInt() - 1;
        const int endChapterID = foundMatch.captured(4).toInt() - 1;
