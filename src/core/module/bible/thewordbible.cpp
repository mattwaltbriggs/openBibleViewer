...
VerseBook TheWordBible::loadCached(const int bookID)
{
    ...
    for(int lineCount = 0; !in.atEnd(); lineCount++) {
        ...
        QRegularExpression strong("<W(G|H)(\\d+)(x|s)?>");
        QRegularExpression newLine("<CL>");
...
void TheWordBible::search(const SearchQuery &query, SearchResult *res) const
{
    ...
    for(auto it = matches.begin(); it != matches.end(); ++it) {
        ...
        if(query.range == SearchQuery::Whole || (query.range == SearchQuery::OT && l.at(0).toInt() <= 38) || (query.range == SearchQuery::NT && l.at(0).toInt() > 38)) {
            ...
            hit.setValue(SearchHit::Score, (double) it.get_weight());
    ...
    foreach(const int bookID, m_versification->bookIDs()) {
        ...
        while(chapterIt.hasNext()) {
            ...
            while(verseIt.hasNext()) {
                ...
                Xapian::Document doc;
                doc.set_data(SearchTools::toStdString(text));
                doc.add_value(0, SearchTools::toStdString(key));
                tg.set_document(doc);
                tg.index_text(SearchTools::toStdString(text));
                db.add_document(doc);
            }
        }
    }
