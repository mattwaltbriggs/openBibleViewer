    ...
    if(file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while(!file.atEnd()) {
            ...
            if(line.contains("BibleName", Qt::CaseInsensitive)) {
                m_moduleName = formatFromIni(line.remove(QRegularExpression("BibleName(\\s*)=(\\s*)", QRegularExpression::CaseInsensitiveOption)));
            }
            else if(line.contains("BibleShortName", Qt::CaseInsensitive)) {
                m_moduleShortName = formatFromIni(line.remove(QRegularExpression("BibleShortName(\\s*)=(\\s*)", QRegularExpression::CaseInsensitiveOption)));
            }
            else if(line.contains("ChapterSign", Qt::CaseInsensitive)) {
                m_chapterSign = formatFromIni(line.remove(QRegularExpression("ChapterSign(\\s*)=(\\s*)", QRegularExpression::CaseInsensitiveOption)));
            }
            else if(line.contains("HTMLFilter", Qt::CaseInsensitive)) {
                m_removeHtml = formatFromIni(line.remove(QRegularExpression("HTMLFilter(\\s*)=(\\s*)", QRegularExpression::CaseInsensitiveOption)));
            }
            else if(line.contains("VerseSign", Qt::CaseInsensitive)) {
                m_verseSign = formatFromIni(line.remove(QRegularExpression("VerseSign(\\s*)=(\\s*)", QRegularExpression::CaseInsensitiveOption)));
            }
            else if(line.contains("ChapterZero", Qt::CaseInsensitive)) {
                const QString zero = formatFromIni(line.remove(QRegularExpression("ChapterZero(\\s*)=(\\s*)", QRegularExpression::CaseInsensitiveOption)));
            ...
            if(started == true) {
                if(started2 == true) {
                    if(line.contains("ChapterQty", Qt::CaseInsensitive)) {
                        bookCount[i] = formatFromIni(line.remove(QRegularExpression("ChapterQty(\\s*)=(\\s*)", QRegularExpression::CaseInsensitiveOption))).toInt();
                        i++;
                        started2 = false;
                    } else if(line.contains("FullName", Qt::CaseInsensitive)) {
                        bookFullName << formatFromIni(line.remove(QRegularExpression("FullName(\\s*)=(\\s*)", QRegularExpression::CaseInsensitiveOption)));

                    } else if(line.contains("ShortName", Qt::CaseInsensitive)) {
                        const QString s = formatFromIni(line.remove(QRegularExpression("ShortName(\\s*)=(\\s*)", QRegularExpression::CaseInsensitiveOption)));
                    ...
                    count++;
                    started2 = true;
                    m_bookPath << formatFromIni(line.remove(QRegularExpression("PathName(\\s*)=(\\s*)", QRegularExpression::CaseInsensitiveOption)));
...
bool BibleQuote::hasIndex() const
{
    ...
    try {
        Xapian::Database db(indexPath().toStdString());
        db.close();
        return true;
    ...
    for(int id = 0; id < m_bookPath.size(); id++) {
        ...
        for(int chapterit = 0; chapterit < ctext.size(); ++chapterit) {
            ...
            for(int verseit = 0; verseit < verses.size(); ++verseit) {
                ...
                Xapian::Document doc;
                doc.set_data(SearchTools::toStdString(t));
                doc.add_value(0, SearchTools::toStdString(key));
                tg.set_document(doc);
                tg.index_text(SearchTools::toStdString(t));
                db.add_document(doc);
            }
        }
    }
...
void BibleQuote::search(const SearchQuery &query, SearchResult *res) const
{
    ...
    Xapian::Query q(SearchTools::toStdString(query.searchText));
    enq.set_query(q);
    Xapian::MSet matches = enq.get_mset(0, 1000);
    for(auto it = matches.begin(); it != matches.end(); ++it) {
        Xapian::Document doc = it.get_document();
        const QString stelle = SearchTools::toQString(doc.get_value(0));
        ...
        if(query.range == SearchQuery::Whole || (query.range == SearchQuery::OT && l.at(0).toInt() <= 38) || (query.range == SearchQuery::NT && l.at(0).toInt() > 38)) {
            ...
            hit.setValue(SearchHit::VerseText, SearchTools::toQString(doc.get_data()));
            res->addHit(hit);
        }
    }
}
