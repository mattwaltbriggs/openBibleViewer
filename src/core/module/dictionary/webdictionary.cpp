...
Response* WebDictionary::getEntry(const QString &entry)
{
    ...
    QJSValue fun = myEngine->evaluate(m_parseScript);
    if (myEngine->hasError()) {
         QJSValue err = myEngine->catchError();
         myWarning() << "uncaught exception:" << err.toString();
     }
    QJSValueList args;
    args << entry;
...
QString WebDictionary::parseUrl(const QUrl &url)
{
    ...
    if (myEngine->hasError()) {
         QJSValue err = myEngine->catchError();
         myWarning() << "uncaught exception in" << m_fileName << ":" << err.toString();
    }
