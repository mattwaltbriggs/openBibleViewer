        ...
        if(n.nodeName() == "meta") {
            ...
            while(!n2.isNull()) {
                ...
                if(n2.nodeName() == "url") {
                    ...
                    m_parseBookScript = n2.firstChild().toCDATASection().data();
                } else if(n2.nodeName() == "parseInChapter") {
                    m_parseChapterScript = n2.firstChild().toCDATASection().data();
                } else if(n2.nodeName() == "parseInVerse") {
                    m_parseVerseScript = n2.firstChild().toCDATASection().data();
...
QString WebCommentary::parseUrl(const QUrl &url)
{
    ...
    QJSValue fun = myEngine->evaluate(m_parseOutScript);
    QJSValueList args;
    args << url.toString();
    QJSValue n = fun.call(args);
...
{
    QJSValue fun = myEngine->evaluate(m_parseBookScript);
    QJSValueList args;
